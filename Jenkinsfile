            ...
            steps{                         
                script { 
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'my-ssh-key', keyFileVariable: "KEY_FILE")]) {                                                
                        sh "ssh -o StrictHostKeyChecking=no -o Protocol=1 -i $KEY_FILE ubuntu@54.166.117.85 \"kubectl set image deployments/myapp-deployment-v2 myimage=ryang123ism/newimage:1.0\" + "${imageTag}""
                    }           
                }
            }                      
        }
